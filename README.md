...
<br/><br/><br/><br/>![image description](https://i.esdrop.com/d/igmccyiogpxf/2F04DGM0Ef.gif)
...
### 2. API Server
     ...
     - /api/portuse: 해운항만물류시스템-Portmis에서 제공하는 API 서버로 부터 POST요청을 통하여 시설사용허가현황 데이터를 수집후 하여 JSON형식으로 데이터를 제공  
     - /api/inout: 해운항만물류시스템-Portmis에서 제공하는 API 서버로부터 POST요청을 통하여 선박입출항정보 데이터를 수집후 정제하여 JSON형식으로 데이터를 제공
